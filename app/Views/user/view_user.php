...
<div class="row">
    <div class="col-sm-12">
        ...
        <table id="example2" class="table table-bordered table-hover dataTable dtr-inline" aria-describedby="example2_info" width="100%">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                ?>
                    <tr class="odd">
                        ...
                        <td>
                            <button type="button" class="btn btn-block btn-success btn-sm" onclick="window.location='<?php echo site_url('user/formedit/' . $row['id']); ?>'">Edit</button>
                            <button type="button" class="btn btn-block btn-danger btn-sm" onclick="hapus(<?= $row['id']; ?>)">Delete</button>
                        </td>
                    </tr>
                <?php
                endforeach;
        ...
        <script>
            function hapus(id) {
                ...
                if (pesan) {
                    window.location.href = ("<?= site_url('user/hapus/') ?>" + id);
                } else return false;
            }
        </script>
    </div>
